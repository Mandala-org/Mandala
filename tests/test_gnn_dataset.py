...
def test_energy_consistency(dataset):
    """Energy / Ne in *y* must reproduce Snapshot helpers."""
    _, _, y = dataset[0]
    snap = y["snapshot"]
    assert torch.allclose(y["energy"], snap.get_energy(), atol=1e-6)
    assert torch.allclose(y["num_electrons"], snap.get_number_of_electrons(), atol=1e-6)
