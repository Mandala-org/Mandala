...
def _normalize_run_config(mapping: Any) -> dict[str, Any]:
    normalized = _normalize_mapping(mapping)
    return {
        key: _normalize_run_config_value(key, value)
        for key, value in normalized.items()
    }


def _normalize_run_config_value(key: str, value: Any) -> Any:
    if isinstance(value, str):
        parsed = _parse_string_literal(value)
        if parsed is not None:
            value = parsed

    if _is_sequence_like_key(key):
        if isinstance(value, str):
            return [value.strip()]
        if isinstance(value, (list, tuple)):
            return [_normalize_sequence_item(item) for item in value]
        if isinstance(value, dict):
            return {k: _normalize_sequence_item(v) for k, v in value.items()}
        return [value]

    if isinstance(value, list):
        return [_normalize_run_config_value(key, item) for item in value]
    if isinstance(value, tuple):
        return tuple(_normalize_run_config_value(key, item) for item in value)
    if isinstance(value, dict):
        return {
            k: _normalize_run_config_value(f"{key}.{k}", v) for k, v in value.items()
        }
    return value
...
def _normalize_sequence_item(value: Any) -> Any:
    ...
    if isinstance(value, list):
        return [_normalize_sequence_item(item) for item in value]
    if isinstance(value, tuple):
        return tuple(_normalize_sequence_item(item) for item in value)
    if isinstance(value, dict):
        return {k: _normalize_sequence_item(v) for k, v in value.items()}
    return value


def _is_sequence_like_key(key: str) -> bool:
