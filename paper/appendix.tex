...
The following listings contain the resolved configuration stored by Weights
\& Biases for each model used in Section~\ref{sec:results}. Run identifiers,
service metadata, and launch commands are omitted. Explicit filesystem paths
are replaced by \texttt{...}. The redundant serialized \texttt{cfg}
representation is also omitted; its settings are retained as structured
entries under \texttt{configuration}.
