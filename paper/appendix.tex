...
\item[\texttt{tp\_type}] Default: \texttt{separate\_weight}. Values: \texttt{separate\_weight} uses the internal separate-weight tensor product, and \texttt{fully\_connected} uses e3nn's implementation.
...
\begin{description}
\item[\texttt{lr}] Default: \texttt{0.010803383053097332}. Learning rate for \texttt{AdamW}.
\item[\texttt{max\_epochs}] Default: \texttt{10000}. Intended training horizon.
...
\item Force- and stress-related fields are retained for experimental development, but their present derivatives are derivatives of the band energy, not the Kohn--Sham total energy, and they are not part of the validated paper-facing objective.
\end{enumerate}
