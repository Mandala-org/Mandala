...
\begin{abstract}
Electronic-structure calculations based on Kohn-Sham density functional theory remain indispensable in computational materials science and chemistry, but their cost limits accessible system sizes, simulation times, and the breadth of architecture and hyperparameter studies that can be carried out on top of such data. At the same time, conventional machine-learning interatomic models usually target only energies and forces, which leaves out the operator-level electronic information required for many scientifically relevant analyses. \texttt{Mandala} was developed to address this gap by providing a framework for learning sparse electronic-structure operators directly, while preserving access to derived observables and electronic post-processing.
...
PySCF are treated as peer electronic-structure backends \cite{ozaki2003,ozakikino2004,blum2009,pyscf2020}. OpenMX and FHI-aims are fully supported, while PySCF data already pass through the same sparse-operator interface [TODO: finish PySCF training-data integration].
...
\in d_0 \times 0e,
\end{equation}
while a Mandala-style edge encoder first forms an edge-type embedding
