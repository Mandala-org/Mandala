...
\texttt{Mandala} is intended to enable machine-learning workflows that go beyond conventional atomistic interatomic potentials by resolving electronic structure together with atomistic response, thereby opening a route toward modeling electronically complex condensed-matter and molecular systems within one scalable framework.
\end{abstract}
...
PySCF are treated as peer electronic-structure backends \cite{ozaki2003,ozakikino2004,blum2009,pyscf2020}, with PySCF integration still under active development [TODO: finish PySCF training-data integration].
...
band-structure analysis within the same sparse-operator workflow
...
edge features. Figure~\ref{fig:model_architecture} summarizes the computational
structure of the model stack, and Figure~\ref{fig:search_space} summarizes the
configuration-driven search space exposed by the software. If $z_i$ denotes the atomic-species index of atom $i$, the
...
\in d_0 \times 0e,
\end{equation}
while one available edge-encoder variant first forms an edge-type embedding
