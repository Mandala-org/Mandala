...
if [[ $# -gt 3 ]]; then
  echo "Usage: $0 [checkpoint_path=${DEFAULT_CHECKPOINT_PATH}] [additional_epochs=1000] [run_name_prefix=long_matrices_restart]" >&2
  exit 1
fi

CHECKPOINT_PATH="${1:-${DEFAULT_CHECKPOINT_PATH}}"
ADDITIONAL_EPOCHS="${2:-1000}"
RUN_NAME_PREFIX="${3:-long_matrices_restart}"
