...
RUN_NAME="${RUN_NAME:-batch1_synth_mixed_$(date +%Y%m%d_%H%M%S)}"
DEVICE="${DEVICE:-cuda}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-studies/e3mlp_investigation/cache/mplconfig}"

echo "[batch1_run03] starting synthetic mixed sweep: ${RUN_NAME}"
python -u studies/e3mlp_investigation/scripts/run_synthetic_teacher_study.py \
  ...
  --run-name "${RUN_NAME}" \
  --append-log
echo "[batch1_run03] finished: ${RUN_NAME}"
