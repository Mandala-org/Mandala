...
RUN_NAME="${RUN_NAME:-batch2_silicon_hamiltonian_attention_$(date +%Y%m%d_%H%M%S)}"
DEVICE="${DEVICE:-cuda}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-studies/e3mlp_investigation/cache/mplconfig}"

echo "[batch2_run02] starting silicon hamiltonian attention sweep: ${RUN_NAME}"
python -u studies/e3mlp_investigation/scripts/run_silicon_nognn_study.py \
  ...
  --message-variant gatemagnitudes \
  --message-depth 2 \
  --predictor-variant gatemagnitudes \
  --predictor-depth 4 \
  ...
  --run-name "${RUN_NAME}" \
  --append-log
echo "[batch2_run02] finished: ${RUN_NAME}"
