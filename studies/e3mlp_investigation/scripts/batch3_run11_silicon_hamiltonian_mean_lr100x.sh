...
RUN_NAME="${RUN_NAME:-batch3_silicon_hamiltonian_mean_lr100x_$(date +%Y%m%d_%H%M%S)}"
DEVICE="${DEVICE:-cuda}"
...
echo "[batch3_run11] starting silicon hamiltonian mean lr100x run: ${RUN_NAME}"
python -u studies/e3mlp_investigation/scripts/run_silicon_nognn_study.py "${ARGS[@]}"
echo "[batch3_run11] finished: ${RUN_NAME}"
