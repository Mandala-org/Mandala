...
RUN_NAME="${RUN_NAME:-batch3_synth_quadratic_prenorm_huber_$(date +%Y%m%d_%H%M%S)}"
DEVICE="${DEVICE:-cuda}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-studies/e3mlp_investigation/cache/mplconfig}"

echo "[batch3_run05] starting synthetic quadratic prenorm huber sweep: ${RUN_NAME}"
python -u studies/e3mlp_investigation/scripts/run_synthetic_teacher_study.py \
  ...
  --pre-norm \
  --run-name "${RUN_NAME}"
echo "[batch3_run05] finished: ${RUN_NAME}"
