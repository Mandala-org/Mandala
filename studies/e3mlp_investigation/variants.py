...
def _chunks(irreps: Irreps) -> list[tuple[int, int, Irrep]]:
    out: list[tuple[int, int, Irrep]] = []
    start = 0
    for mul, ir in Irreps(irreps):
        size = mul * ir.dim
        out.append((start, size, ir))
        start += size
    return out
...
class ScaledLinear(nn.Module):
    ...
    def irreps_in(self) -> Irreps:
        return self.linear.irreps_in

    @property
    def irreps_out(self) -> Irreps:
        return self.linear.irreps_out
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for mul, ir in self.irreps:
            ...
            if ir.l == 0:
                ...
                if ir.p == 1:
                    block = self.scalar_act(block)
                else:
                    block = self.odd_scalar_act(block)
...
def make_gate_activation(
    ...
    *,
    ...
    scalar_acts: list[Callable[[torch.Tensor], torch.Tensor]] = []
    for _, ir in irreps_scalars:
        if ir.p == 1:
            scalar_acts.append(scalar_activation_fn(scalar_activation))
        else:
            scalar_acts.append(odd_safe_activation(odd_scalar_activation))

    gate_acts: list[Callable[[torch.Tensor], torch.Tensor]] = []
    for _, ir in irreps_gates:
        if ir.p == 1:
            gate_acts.append(scalar_activation_fn(gate_activation))
        else:
            gate_acts.append(odd_safe_activation(odd_gate_activation))
...
class BilinearSelfTPBlock(nn.Module):
    def __init__(
        ...
        irreps_hidden: Irreps,
        *,
        interaction_irreps: Irreps | None = None,
        output_scale: float = 1.0,
        weight_init_scale: float = 1.0,
...
class SimpleE3MLP(nn.Module):
    ...
    def _make_hidden_block(self, irreps_in: Irreps, cfg: VariantConfig) -> nn.Module:
        ...
        if variant == "resnormact":
            ...
            if irreps_in != self.irreps_hidden:
            ...
            return ResidualBlock(inner, self.irreps_hidden.dim, cfg.residual_scale)
