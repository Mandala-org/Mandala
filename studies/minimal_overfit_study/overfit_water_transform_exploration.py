"""
Transformation Exploration Study: Test Different Data Loading Variants
=======================================================================

This script explores different transformation variants in the OpenMX → E3NN
pipeline to identify potential bugs. It's based on overfit_water_minimal.py
but allows configurable coordinate permutations, SH basis transforms, etc.

Focus: Validate transformations by tracking 1e irrep (axial vectors) loss,
       which is known to be problematic in the standard pipeline.

Architecture: Same as overfit_water_minimal.py
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="2,0,1",
        choices=["2,0,1", "1,2,0", "none"],
        help="Coordinate permutation: '2,0,1' (current), '1,2,0' (inverse), 'none'",
    )
    parser.add_argument(
        "--box-permutation",
        type=str,
        default=None,
        choices=["same", "none"],
        help="Box permutation: 'same' (follow coords), 'none' (no permutation). Default: same as coords",
    )
    parser.add_argument(
        "--sh-transform-mode",
        type=str,
        default="standard",
        choices=["standard", "inverse", "none"],
        help="SH basis transformation: 'standard' (U@H@U.T), 'inverse' (U.T@H@U), 'none' (identity)",
    )
    parser.add_argument(
        "--sh-l1-override",
        type=str,
        default=None,
        choices=["2,0,1", "1,2,0", "none"],
        help="Override SH transform for l=1 (p orbitals): '2,0,1', '1,2,0', or 'none'",
    )
    parser.add_argument(
        "--mirror-x",
        action="store_true",
    ...
    CONFIG = {
        ...
        "coord_permutation": args.coord_permutation,
        "box_permutation": args.box_permutation,
        "sh_transform_mode": args.sh_transform_mode,
        "sh_l1_override": args.sh_l1_override,
    ...
    # Initialize WandB
    from transform_utils import get_transform_config_str

    transform_suffix = get_transform_config_str(
        args.coord_permutation,
        args.box_permutation,
        args.sh_transform_mode,
        args.sh_l1_override,
        args.mirror_x,
    )

    wandb_kwargs = {
        "project": "mandala-transform-exploration",
        "config": CONFIG,
        "tags": [
            f"coord_{args.coord_permutation}",
            f"sh_{args.sh_transform_mode}",
        ],
    }
    if args.sh_l1_override:
        wandb_kwargs["tags"].append(f"l1_{args.sh_l1_override}")
    if args.mirror_x:
        wandb_kwargs["tags"].append("mirror_x")

    if args.run_name is not None:
        wandb_kwargs["name"] = f"{args.run_name}_{transform_suffix}"
    else:
        wandb_kwargs["name"] = transform_suffix
    ...
    print(f"  Coordinate permutation: {CONFIG['coord_permutation']}")
    print(f"  Box permutation: {CONFIG['box_permutation'] or 'same as coords'}")
    print(f"  SH transform mode: {CONFIG['sh_transform_mode']}")
    if CONFIG["sh_l1_override"]:
        print(f"  SH l=1 override: {CONFIG['sh_l1_override']}")
    if CONFIG["mirror_x"]:
        print(f"  Mirror X: enabled")
    ...
    # Load snapshot with configurable transformations
    print(f"  Matrix file: {CONFIG['data_path']}")
    print(f"  Info file: {CONFIG['info_path']}")

    from transform_utils import load_snapshot_with_transforms

    snapshot = load_snapshot_with_transforms(
        matrix_path=CONFIG["data_path"],
        info_path=CONFIG["info_path"],
        coord_permutation=CONFIG["coord_permutation"],
        box_permutation=CONFIG["box_permutation"],
        sh_transform_mode=CONFIG["sh_transform_mode"],
        sh_l1_override=CONFIG["sh_l1_override"],
    ...
    for epoch in range(CONFIG["num_epochs"]):
        ...
        if CONFIG["train_on_irrep_parts"]:
            for irrep_str, irrep_loss in irrep_losses.items():
                ...
                # Special tracking for 1e irrep (axial vectors - known problematic)
                if irrep_str == "1e":
                    step_log["validation/1e_loss"] = irrep_loss.item()
        if epoch % CONFIG["log_interval"] == 0:
