...
def log_config(config: dict, run_checkpoint_dir, frame_output_dir) -> None:
    ...
    print(
        "  Head E3MLP: "
        f"{config.get('head_e3mlp_variant', 'gate')} "
        f"(layers={config.get('head_e3mlp_layers', 1)})"
    )
    print(
        "  Internal E3MLP: "
        f"{config.get('internal_e3mlp_variant', 'gate')} "
        f"(layers={config.get('internal_e3mlp_layers', 0)})"
    )
    print(
        "  E3MLP init/norm: "
        f"pre_norm={config.get('e3mlp_pre_norm', False)}, "
        f"output_scale={config.get('e3mlp_output_scale', 1.0)}, "
        f"weight_init_scale={config.get('e3mlp_weight_init_scale', 1.0)}, "
        f"residual_scale={config.get('e3mlp_residual_scale', 0.25)}"
    )
    print(
        "  Head branch scales: "
        f"diag={config.get('head_diag_output_scale', 1.0)}, "
        f"offdiag={config.get('head_offdiag_output_scale', 1.0)}"
    )
    print(
