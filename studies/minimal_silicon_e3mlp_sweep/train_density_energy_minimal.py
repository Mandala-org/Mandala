...
ARCHITECTURE_ARG_NAMES = {
    ...
    "head_e3mlp_layers",
    "head_e3mlp_variant",
    "internal_e3mlp_variant",
    "internal_e3mlp_layers",
    "e3mlp_pre_norm",
    "e3mlp_output_scale",
    "e3mlp_weight_init_scale",
    "e3mlp_residual_scale",
    "e3mlp_film_hidden_dim",
    "head_diag_output_scale",
    "head_offdiag_output_scale",
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=parse_matrix_targets("density"),
    ...
    parser.add_argument(
        ...
        type=parse_bool,
        default=False,
        help="Enable number-of-electrons metric computation from predicted matrices (default: False).",
    )
    parser.add_argument(
        "--train-on-energy",
        type=parse_bool,
        default=False,
        help="Enable energy loss term (default: False).",
    )
    parser.add_argument(
        "--train-on-num-electrons",
        type=parse_bool,
        default=False,
        help="Enable number-of-electrons loss term (default: False).",
    )
    parser.add_argument(
    ...
    parser.add_argument("--head-e3mlp-layers", type=int, default=2)
    parser.add_argument(
        "--head-e3mlp-variant",
        type=str,
        default="gatemagnitudes",
        choices=E3MLP_VARIANT_CHOICES,
    )
    parser.add_argument(
        "--internal-e3mlp-variant",
        type=str,
        default="resgatemagnitudes",
        choices=E3MLP_VARIANT_CHOICES,
    )
    parser.add_argument("--internal-e3mlp-layers", type=int, default=1)
    parser.add_argument("--e3mlp-pre-norm", type=parse_bool, default=True)
    parser.add_argument("--e3mlp-output-scale", type=float, default=0.5)
    parser.add_argument("--e3mlp-weight-init-scale", type=float, default=0.25)
    parser.add_argument("--e3mlp-residual-scale", type=float, default=0.05)
    parser.add_argument("--e3mlp-film-hidden-dim", type=int, default=128)
    parser.add_argument("--head-diag-output-scale", type=float, default=1.0)
    parser.add_argument("--head-offdiag-output-scale", type=float, default=0.1)
    ...
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
        default="studies/minimal_silicon_e3mlp_sweep/checkpoints",
    )
    parser.add_argument(
        "--wandb-project",
        type=str,
        default="mandala-minimal-silicon-e3mlp-sweep",
...
def compute_loss_for_sample(
    ...
    *,
    ...
    for matrix_name in matrix_targets:
        ...
        pred_matrix = align_pred_block_matrix_to_target_edges(
    ...
    if enable_energy and "density" in pred_matrix_observables_by_name:
        energy_hamiltonian = pred_matrix_observables_by_name.get(
            "hamiltonian", sample["target_hamiltonian"]
        )
        energy_alignment = build_trace_alignment(
            energy_hamiltonian, pred_matrix_observables_by_name["density"]
        )
        ...
        if train_on_energy and "hamiltonian" in pred_matrix_observables_by_name:
...
def predict_sample(
    ...
    *,
    ...
    pred_matrix_metrics_by_name = {
        matrix_name: align_pred_block_matrix_to_target_edges(
            pred_matrix_metrics_by_name[matrix_name],
            sample["target_matrices"][matrix_name],
        )
        for matrix_name in matrix_targets
    }
    pred_irreps_metrics_by_name = {
        matrix_name: align_pred_irreps_to_target_edges(
...
def evaluate_split(
    ...
    *,
    ...
    for idx, sample in enumerate(samples):
        ...
        with torch.no_grad():
            if (
                "hamiltonian" in pred_metrics_by_name
                and "hamiltonian" in target_matrices_detached
            ):
...
def main() -> None:
    ...
    if args.internal_e3mlp_layers < 0:
        raise ValueError("--internal-e3mlp-layers must be >= 0.")
    ...
    if args.train_on_energy and "density" not in matrix_targets:
        raise ValueError("--train-on-energy requires density in --matrix-targets.")
    ...
    config = {
        ...
        "head_e3mlp_variant": args.head_e3mlp_variant,
        "internal_e3mlp_variant": args.internal_e3mlp_variant,
        "internal_e3mlp_layers": args.internal_e3mlp_layers,
        "e3mlp_pre_norm": args.e3mlp_pre_norm,
        "e3mlp_output_scale": args.e3mlp_output_scale,
        "e3mlp_weight_init_scale": args.e3mlp_weight_init_scale,
        "e3mlp_residual_scale": args.e3mlp_residual_scale,
        "e3mlp_film_hidden_dim": args.e3mlp_film_hidden_dim,
        "head_diag_output_scale": args.head_diag_output_scale,
        "head_offdiag_output_scale": args.head_offdiag_output_scale,
    ...
    primary_matrix_name = (
        "density"
        if "density" in matrix_targets
        else ("hamiltonian" if "hamiltonian" in matrix_targets else matrix_targets[0])
    )
    primary_matrix_alias = matrix_alias.get(primary_matrix_name, primary_matrix_name)
    ...
    history = {
        "train_loss": [],
        "val_loss": [],
        f"val_mae_{primary_matrix_alias}": [],
    }
    ...
    if resume_payload is not None:
        ...
        if primary_matrix_name == "hamiltonian":
            ...
            print("\n[INITIAL METRICS]")
            print(f"  Loss: {float(initial_eval['loss']):.6e}")
            if primary_matrix_name in basic_by_name:
                print(
                    f"  MAE {primary_matrix_alias}: {basic_by_name[primary_matrix_name]['mae']:.6e}"
                )
                print(
                    f"  MSE {primary_matrix_alias}: {basic_by_name[primary_matrix_name]['mse']:.6e}"
                )
        ...
        if primary_matrix_name in basic_by_name:
            initial_wandb_log["initial/mae_primary"] = basic_by_name[
                primary_matrix_name
            ]["mae"]
            initial_wandb_log["initial/mse_primary"] = basic_by_name[
                primary_matrix_name
            ]["mse"]
    ...
    try:
        for epoch in range(start_epoch, train_end_epoch):
            ...
            history[f"val_mae_{primary_matrix_alias}"].append(
                (
                    float(
                        val_eval["basic_by_name"]
                        .get(primary_matrix_name, {})
                        .get("mae", float("nan"))
                    )
                    if do_log
                    else float("nan")
            ...
            if do_log:
                ...
                if primary_matrix_name == "hamiltonian":
                    ...
                    print(f"  Loss:                 {train_loss:.6e}")
                    if primary_matrix_name in val_eval["basic_by_name"]:
                        print(
                            f"  MAE {primary_matrix_alias}:                "
                            f"{val_eval['basic_by_name'][primary_matrix_name]['mae']:.6e}"
                        )
                        print(
                            f"  MSE {primary_matrix_alias}:                "
                            f"{val_eval['basic_by_name'][primary_matrix_name]['mse']:.6e}"
                        )
                ...
                if primary_matrix_name in basic_by_name:
                    epoch_wandb_log["mae_primary"] = basic_by_name[primary_matrix_name][
                        "mae"
                    ]
                    epoch_wandb_log["mse_primary"] = basic_by_name[primary_matrix_name][
                        "mse"
                    ]
                if (
    ...
    if primary_matrix_name == "hamiltonian":
        ...
        print(f"\n[FINAL PREDICTIONS - {primary_matrix_name.capitalize()}]")
        print("\n  Overall Metrics:")
        final_metrics = {}
    ...
    if primary_matrix_name in basic_by_name and primary_matrix_name != "hamiltonian":
        print(
            f"    MAE {primary_matrix_alias}:                "
            f"{basic_by_name[primary_matrix_name]['mae']:.6e}"
        )
        print(
            f"    MSE {primary_matrix_alias}:                "
            f"{basic_by_name[primary_matrix_name]['mse']:.6e}"
        )
        final_metrics["final/mae_primary"] = basic_by_name[primary_matrix_name]["mae"]
        final_metrics["final/mse_primary"] = basic_by_name[primary_matrix_name]["mse"]
