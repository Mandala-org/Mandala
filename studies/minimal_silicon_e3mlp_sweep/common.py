...
from studies.e3mlp_investigation.variants import build_variant
...
class MinimalMessageBlock(nn.Module):
    ...
    def __init__(
        ...
        layer_idx=0,
        use_e3layernorm=True,
        internal_e3mlp_variant="gate",
        internal_e3mlp_layers=0,
        internal_pre_norm=False,
        internal_output_scale=1.0,
        internal_weight_init_scale=1.0,
        internal_residual_scale=0.25,
        internal_film_hidden_dim=128,
    ):
        super().__init__()
        ...
        self.use_e3layernorm = bool(use_e3layernorm)
        self.internal_e3mlp_variant = str(internal_e3mlp_variant)
        self.internal_e3mlp_layers = int(internal_e3mlp_layers)
        ...
        if self.internal_e3mlp_layers > 0:
            self.node_refine = E3VariantMLP(
                self.node_gate.irreps_out,
                hidden_irreps,
                hidden_irreps,
                variant=self.internal_e3mlp_variant,
                num_layers=self.internal_e3mlp_layers,
                output_scale=internal_output_scale,
                weight_init_scale=internal_weight_init_scale,
                residual_scale=internal_residual_scale,
                pre_norm=internal_pre_norm,
                film_hidden_dim=internal_film_hidden_dim,
            )
            self.edge_refine = E3VariantMLP(
            ...
            self.node_refine = nn.Identity()
            self.edge_refine = nn.Identity()
        ...
        if self.internal_e3mlp_layers > 0:
            print(
                "                   -> Internal E3MLP refine: "
                f"{self.internal_e3mlp_variant} layers={self.internal_e3mlp_layers}"
            )
    ...
    def forward(
        ...
        node_feat_new = self.node_refine(node_feat_new)
        check_for_nans(node_feat_new, f"MessageBlock[{self.layer_idx}].node_refine")
        ...
        edge_feat_new = self.edge_refine(edge_feat_new)
        check_for_nans(edge_feat_new, f"MessageBlock[{self.layer_idx}].edge_refine")
...
class E3VariantMLP(nn.Module):
    """Wrapper around the E3MLP study zoo with optional branch scaling."""
    ...
    def __init__(
        ...
        *,
        ...
        self.model = build_variant(
            ...
            weight_init_scale=float(weight_init_scale),
    ...
    def forward(self, x):
        return self.post_scale * self.model(x)
...
class MinimalHead(nn.Module):
    ...
    def __init__(
        ...
        n_radial,
        head_e3mlp_layers=1,
        head_e3mlp_variant="gate",
        magnitude_factorization=False,
        head_mlp_for_scalars=False,
        head_use_tensor_square=False,
        separate_shifted_self=False,
        head_pre_norm=False,
        head_output_scale=1.0,
        head_weight_init_scale=1.0,
        head_residual_scale=0.25,
        head_film_hidden_dim=128,
        head_diag_output_scale=1.0,
        head_offdiag_output_scale=1.0,
        head_shifted_self_output_scale=None,
    ):
        super().__init__()
        ...
        self.head_e3mlp_variant = str(head_e3mlp_variant)
        self.head_diag_output_scale = float(head_diag_output_scale)
        self.head_offdiag_output_scale = float(head_offdiag_output_scale)
        self.head_shifted_self_output_scale = (
            float(head_shifted_self_output_scale)
            if head_shifted_self_output_scale is not None
            else self.head_diag_output_scale
        )
        ...
        for edge_type in self.edge_types:
            ...
            if projection_irreps.dim > 0:
                self.diag_projections[edge_type] = E3VariantMLP(
                    head_input_irreps,
                    head_input_irreps,
                    projection_irreps,
                    variant=self.head_e3mlp_variant,
                    num_layers=self.head_e3mlp_layers,
                    output_scale=head_output_scale,
                    weight_init_scale=head_weight_init_scale,
                    residual_scale=head_residual_scale,
                    pre_norm=head_pre_norm,
                    film_hidden_dim=head_film_hidden_dim,
                    post_scale=self.head_diag_output_scale,
                )
                if self.separate_shifted_self:
                    self.shifted_self_projections[edge_type] = E3VariantMLP(
                        head_input_irreps,
                        head_input_irreps,
                        projection_irreps,
                        variant=self.head_e3mlp_variant,
                        ...
                        pre_norm=head_pre_norm,
                        film_hidden_dim=head_film_hidden_dim,
                        post_scale=self.head_shifted_self_output_scale,
                    )
                self.offdiag_projections[edge_type] = E3VariantMLP(
                    head_input_irreps,
                    head_input_irreps,
                    projection_irreps,
                    variant=self.head_e3mlp_variant,
                    ...
                    pre_norm=head_pre_norm,
                    film_hidden_dim=head_film_hidden_dim,
                    post_scale=self.head_offdiag_output_scale,
                )
            ...
            print(
                "            head projections: "
                f"{self.head_e3mlp_variant} layers={self.head_e3mlp_layers} "
                f"(diag_scale={self.head_diag_output_scale}, "
                f"offdiag_scale={self.head_offdiag_output_scale})"
            )
...
class MinimalNetwork(nn.Module):
    ...
    def __init__(
        ...
        separate_shifted_self=False,
        use_e3layernorm=True,
        e3mlp_pre_norm=False,
        e3mlp_output_scale=1.0,
        e3mlp_weight_init_scale=1.0,
        e3mlp_residual_scale=0.25,
        e3mlp_film_hidden_dim=128,
        head_diag_output_scale=1.0,
        head_offdiag_output_scale=1.0,
        verbose=True,
    ):
        ...
        for i in range(num_layers):
            ...
            self.mp_layers.append(
                MinimalMessageBlock(
                    ...
                    layer_idx=i,
                    use_e3layernorm=use_e3layernorm,
                    internal_e3mlp_variant=internal_e3mlp_variant,
                    internal_e3mlp_layers=internal_e3mlp_layers,
                    internal_pre_norm=e3mlp_pre_norm,
                    internal_output_scale=e3mlp_output_scale,
                    internal_weight_init_scale=e3mlp_weight_init_scale,
                    internal_residual_scale=e3mlp_residual_scale,
                    internal_film_hidden_dim=e3mlp_film_hidden_dim,
                )
            )
        ...
        self.heads = nn.ModuleDict(
            {
                matrix_name: MinimalHead(
                    ...
                    head_e3mlp_variant=head_e3mlp_variant,
                    ...
                    head_pre_norm=e3mlp_pre_norm,
                    head_output_scale=e3mlp_output_scale,
                    head_weight_init_scale=e3mlp_weight_init_scale,
                    head_residual_scale=e3mlp_residual_scale,
                    head_film_hidden_dim=e3mlp_film_hidden_dim,
                    head_diag_output_scale=head_diag_output_scale,
                    head_offdiag_output_scale=head_offdiag_output_scale,
                )
