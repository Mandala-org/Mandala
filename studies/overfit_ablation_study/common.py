...
class MinimalEdgeEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.use_e3layernorm = bool(use_e3layernorm) and str(norm_kind) != "none"
        ...
        # Optional equivariant layer norm (kept configurable for ablation study)
        if self.use_e3layernorm:
            self.norm = e3LayerNorm(
                self.irreps_out, normalization=norm_kind, verbose=False
            )
        else:
        ...
        print(
            "                  Norm: "
            + (
                f"e3LayerNorm({self.irreps_out}, normalization={norm_kind})"
                if self.use_e3layernorm
                else "disabled"
            )
        )
...
class MinimalMessageBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_e3layernorm:
            self.edge_norm = e3LayerNorm(
                self.edge_gate.irreps_out, normalization=norm_kind, verbose=False
            )
        else:
        ...
        if self.use_e3layernorm:
            self.node_norm = e3LayerNorm(
                self.node_gate.irreps_out, normalization=norm_kind, verbose=False
            )
        else:
    ...
    def forward(
        ...
        if self.skip_connections and node_feat_new.shape == node_residual.shape:
            node_feat_new = node_feat_new + node_residual
        ...
        if self.skip_connections and edge_feat_new.shape == edge_residual.shape:
            edge_feat_new = edge_feat_new + edge_residual
...
class MinimalNetwork(nn.Module):
    ...
    def __init__(
        ...
        print(
            f"  e3LayerNorm: {'enabled' if (use_e3layernorm and norm_kind != 'none') else 'disabled'}"
        )
        print(f"  norm_kind: {norm_kind}")
        print(f"  skip_connections: {'enabled' if skip_connections else 'disabled'}")
